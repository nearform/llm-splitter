...
- **A literal U+FFFD in your source** — scraped and mojibake-recovered text often contains one. Because tier 2 is skipped for U+FFFD-bearing parts, a manufactured U+FFFD can no longer match an unrelated literal one downstream, which is what used to throw on ordinary RAG input. Two narrow effects remain. **First**, tier 1 tests only whether the source begins with the part at the cursor, and there a manufactured bare U+FFFD is byte-identical to a literal one. So a literal U+FFFD sitting exactly at the cursor may be claimed by a manufactured part. That adds a chunk boundary at that position and attributes one code unit to the wrong token; because both are one code unit wide the cursor still advances correctly, so nothing after it shifts and coverage is unaffected. **Second**: because tier 2 is skipped, a part that mixes U+FFFD with real text and _does_ occur verbatim in the source is positioned by tier 3's grapheme search rather than by exact match. Each candidate occurrence is now verified — the part's non-U+FFFD code units must line up with the source there — so a part whose anchor grapheme also occurs in the span the splitter dropped no longer anchors on that earlier occurrence. Measured against a real string-delimiter splitter over U+FFFD-bearing text, that takes the class from 10.5% of documents mis-anchored to 0%, at every replacement-char density tested from 0.05% to 17%. What verification cannot resolve is a part that is _mostly_ U+FFFD with only one or two real code units, since U+FFFD positions constrain nothing and an earlier offset can align on all of them; that residual is 158 of 15,986 randomized cases. Where it happens nothing is dropped — coverage and the `chunk.text === getChunk(input, start, end)` correspondence both hold — the boundary just sits a few code units early, so those characters join the following chunk instead of the preceding one. Raising `chunkOverlap` reduces the practical effect further, since the shift is a handful of code units while the overlap is whole tokens.
