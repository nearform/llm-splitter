...
- ✅ `tiktoken` (OpenAI cl100k, ada-002, gpt-4o, etc.) — substitutes exactly one U+FFFD per undecodable byte, so length matches source span. This holds whether or not your source itself contains literal U+FFFD, with the one narrow exception described under "Multibyte / Unicode Strings" above (a literal U+FFFD standing at the cursor may be claimed by a manufactured part, costing one chunk boundary and no drift).
