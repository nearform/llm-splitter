...
2. **`indexOf(part)` forward from cursor** — byte-preserving splitter that drops bytes between parts (e.g. `text.split(/\s+/)` discards whitespace, so the cursor lands in the gap and tier 1 fails). The whole part still exists verbatim in source, so a native substring search finds it without allocating. This tier is **skipped for any part containing U+FFFD**: that character is one the splitter invented, so finding it verbatim somewhere downstream says nothing about where the part came from. Acting on such a match is what used to strand the cursor past real source (see the third failure mode below).
3. **`indexOf(firstAnchorGrapheme(part))` forward from cursor** — byte-mutating splitter (e.g. `tiktoken` emitting U+FFFD when a token straddles a multi-byte sequence). Walk the part's [`Intl.Segmenter`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Intl/Segmenter) graphemes to find the first one that is _anchorable_ (not U+FFFD, not a combining mark or variation selector); locate that grapheme in input, then set `start` to the part's own left edge by subtracting the grapheme's offset within the part. `end` is `start + part.length` (clamped to input length). The offset subtraction is what keeps those two consistent — a part 3 code units wide whose anchor sits 1 unit in starts 1 before the match, not at it. indexOf is safe here because anchor graphemes are full grapheme clusters that, by construction, never start with a low surrogate or combining mark, so the _match_ cannot land mid-surrogate. Subtracting the offset can still put `start` inside a surrogate pair; chunk boundaries carry no code-point integrity guarantee either way (see "Chunk Coverage and Positions").

Three failure modes worth knowing about:

- **Unanchorable parts** — if the entire part consists of U+FFFD and/or combining marks, the part is silently dropped. Usually that means the tokenizer produced nothing positionable, but not always: when the source itself holds a literal U+FFFD, such a part _is_ positionable and is still dropped, because a bare U+FFFD is byte-identical whether the splitter invented it or passed it through. Earlier versions anchored it, so a chunk boundary can disappear relative to them. The source bytes it represented are still preserved in chunk text, because chunks span from their first part's `start` to their last part's `end` and gaps between parts are absorbed forward by `getChunk` (see "Chunk Coverage and Positions" above).
...
- **A literal U+FFFD in your source** — scraped and mojibake-recovered text often contains one. Because tier 2 is skipped for U+FFFD-bearing parts, a manufactured U+FFFD can no longer match an unrelated literal one downstream, which is what used to throw on ordinary RAG input. Two narrow effects remain. **First**, tier 1 tests only whether the source begins with the part at the cursor, and there a manufactured bare U+FFFD is byte-identical to a literal one. So a literal U+FFFD sitting exactly at the cursor may be claimed by a manufactured part. That adds a chunk boundary at that position and attributes one code unit to the wrong token; because both are one code unit wide the cursor still advances correctly, so nothing after it shifts and coverage is unaffected. **Second, and the one to watch**: because tier 2 is skipped, a part that mixes U+FFFD with real text and _does_ occur verbatim in the source is positioned by tier 3's grapheme search instead of by exact match. If that grapheme also occurs between the cursor and the part's true position, the part anchors on the earlier occurrence — a wrong position with no error, which is the failure mode this section warns about above. Reaching it needs a splitter whose dropped span contains a character that can also _begin_ a part, so the exposed shapes are multi-character **string** delimiters — `text.split("\n\n")`, `text.split(". ")`, `text.split("...")` — where a lone `\n` is not `"\n\n"`. Character-class regex splitters are not exposed, however much they drop: a part containing a class member would itself have been split there, so no part can begin with a character the dropped span contains. That covers `text.split(/[.!?]+/)` and `text.split(/\s+/)`, both measured at 0 affected cases. Nothing is dropped when this happens — coverage and the `chunk.text === getChunk(input, start, end)` correspondence both still hold — the boundary just sits a few code units early, so those characters join the following chunk instead of the preceding one. Measured over 15,986 randomized cases in that class, the displacement was 2-3 code units in the large majority and 6 at worst, and in 92% of them only the discarded separator moved; the rest clipped the tail of the preceding token. **Raising `chunkOverlap` all but removes the practical effect**: the shift is a handful of code units while the overlap is whole tokens, so anything that moves across the boundary is already duplicated into the next chunk. Measured by counting tokens left whole in no single chunk — at `chunkSize: 8` that went 275 at `chunkOverlap: 0` to 2 at `chunkOverlap: 2` and 0 at 4; at `chunkSize: 4` it went 705 to 2, so a small chunk size still leaves a residue.
