...
- [ ] 3.4 **Delta already rebased** onto `literal-replacement-char-anchoring` (archived 2026-08-21): this change's `Supported tokenizer boundary` block now carries that change's "Length-preserving tokenizer over a source containing U+FFFD" scenario, without which `validate --strict` fails. Keep Tiers 1–2 byte-identical when `sourceNormalize` is identity, including the length-based advance (no regression on char/whitespace/tiktoken). "Identical" means identical to `main` at rebase time, **not** to what this change was written against: if `literal-replacement-char-anchoring` has landed, Tier 2 is skipped for every U+FFFD-bearing part and Tier 3 subtracts the anchor offset; if `decoy-grapheme-anchoring` has landed (it did, 2026-08-21), Tier 3 also verifies each candidate with `skeletonAligns` before accepting it. Preserve all three on the identity path — but **do not carry `skeletonAligns` onto the normalized path**: it compares part and source at fixed offsets, which is exactly the assumption length inflation breaks, so it would reject every correct candidate. Its own design says so (that change's Risks → "Overlap with `tokenizer-length-inflation`"). Preserve them — that change's regressions will catch a revert, but only if this change rebases onto them rather than over them
...
- [ ] 4.6 **Handed off from `literal-replacement-char-anchoring`.** With `@huggingface/transformers` back (4.2), run the gte-small fixtures against that change's Tier 3 offset correction on the **identity path** — `gteSmallSplitterNaive` with no `sourceNormalize`. That change could not run this leg and explicitly did not record it as passing; see its `design.md` → Risks, "Not verified against `gte-small`". Two things to establish: that the corrected Tier 3 does not turn a loud throw into a silent mis-anchor for these tokenizers, and what the "Phase 1 evidence" table in `research.md` looks like after it (those rows predate the fix). Re-measure rather than assuming — AGENTS.md's rule exists because a synthetic-passing change broke tiktoken on Devanagari (4.4). **Predicted result: both edits are inert for gte-small**, so the Phase 1 table should reproduce unchanged. The offset correction only bites when the part's first anchorable grapheme sits at offset `k > 0`, which requires the part to _begin_ with U+FFFD or a combining mark; every gte-small part in the table (`[CLS]`, `hi`, `there`, `##ん`, `evan`, …) has its anchor at offset 0. The Tier 2 skip only bites on parts containing U+FFFD, and normalizing tokenizers emit none — they mutate case and accents and add `##` literals instead. A table that reproduces confirms the prediction; a table that moves means the reasoning above is wrong and is the finding
