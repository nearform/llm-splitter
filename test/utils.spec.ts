...
describe('chunkByParagraph', () => {
  ...
  test('handles overlap with smaller chunk size', () => {
    ...
    const result: ChunkResult[] = Array.from(
      chunkByParagraph(units, defaultSplitter, 2, 1)
    )
  ...
  test('handles unit larger than chunk size that forces sub-paragraph chunking', () => {
    ...
    const result: ChunkResult[] = chunkByParagraph(units, defaultSplitter, 5, 0)
  ...
  test('handles large unit with custom splitter that forces sub-paragraph chunking', () => {
    const units: ChunkUnit[] = [{ unit: 'verylongword', start: 0, end: 12 }]
    const charSplitter: (text: string) => string[] = (text: string) =>
      text.split('')
    // Unit has 12 characters, chunk size is 5, should create multiple chunks
    const result: ChunkResult[] = chunkByParagraph(units, charSplitter, 5, 0)
    assert.ok(result.length >= 2) // Should have multiple chunks
    assert.ok(result[0].text.length <= 5) // First chunk should respect size limit
  })
  ...
  test('handles CommonJS ESM dependency text with chunk size 200', () => {
    ...
    const units: ChunkUnit[] = [{ unit: text, start: 0, end: text.length }]
    const result: ChunkResult[] = Array.from(
      chunkByParagraph(units, defaultSplitter, 200, 0)
    )
  ...
  test('handles CommonJS ESM dependency text with smaller chunk size to force chunking', () => {
    ...
    const result: ChunkResult[] = Array.from(
      chunkByParagraph(units, defaultSplitter, 50, 0)
    )
    assert.ok(result.length >= 2) // Should have multiple chunks
  ...
  test('handles sub-paragraph chunking with overlap', () => {
    ...
    const units: ChunkUnit[] = [{ unit: text, start: 0, end: text.length }]
    const result: ChunkResult[] = Array.from(
      chunkByParagraph(units, defaultSplitter, 30, 5)
    )
    assert.ok(result.length >= 2) // Should have multiple chunks
  ...
  test('demonstrates complete sub-paragraph chunking with word-based tokenization', () => {
    ...
    const result: ChunkResult[] = Array.from(
      chunkByParagraph(units, wordSplitter, 10, 2)
    )
    assert.ok(result.length >= 2) // Should have multiple chunks
  ...
  test('works with tiktoken as custom splitter', () => {
    const encoding: Tiktoken = encoding_for_model('gpt2')
    ...
    const firstChunk: ChunkResult = result[0]
    if (
  ...
  test('handles comprehensive blog post content with tiktoken tokenization', () => {
    ...
    const units: ChunkUnit[] = getUnits(blogPost)
    ...
    const result: ChunkResult[] = Array.from(
      chunkByParagraph(units, tokenSplitter, 512, 10)
    )
    ...
    for (let i: number = 1; i < result.length; i++) {
      ...
      const currentChunk: ChunkResult = result[i]
      assert.ok(
    ...
    for (let i: number = 1; i < result.length; i++) {
      ...
      if (currentChunk.start < prevChunk.end) {
        ...
        const overlapStart: number = currentChunk.start
        const overlapEnd: number = prevChunk.end
        ...
        assert.ok(overlapTokenCount >= 1, `Chunk ${i} should have some overlap`)
        assert.ok(
  ...
  test('prevents duplicate final chunks when content is already covered by overlap', () => {
    ...
    // Use a smaller chunk size to force multiple chunks
    const result: ChunkResult[] = Array.from(
      chunkByParagraph(units, defaultSplitter, 18, 5)
    )
    ...
    for (let i: number = 0; i < result.length; i++) {
      for (let j: number = i + 1; j < result.length; j++) {
        const chunkA: ChunkResult = result[i]
        const chunkB: ChunkResult = result[j]

        // Neither chunk should be entirely contained within the other
        const aContainsB: boolean =
          chunkA.start <= chunkB.start && chunkA.end >= chunkB.end
        const bContainsA: boolean =
          chunkB.start <= chunkA.start && chunkB.end >= chunkA.end
    ...
    for (let i: number = 1; i < result.length; i++) {
      const current: ChunkResult = result[i]
      const previous: ChunkResult = result[i - 1]

      // Chunks should have overlap (current starts before previous ends)
      if (current.start < previous.end) {
        const overlapSize: number = previous.end - current.start
        assert.ok(
          overlapSize > 0,
  ...
  test('demonstrates paragraph chunking overlap behavior with whole paragraphs', () => {
    ...
    const units: ChunkUnit[] = [
      ...
      {
        unit: 'This is a much longer paragraph that contains significantly more tokens and content than the previous one, making it a good example for testing overlap behavior when working with whole paragraph units in the chunking system.',
        start: 39,
        end: 238
      },
    ...
    const tokenSplitter: (text: string) => string[] = (
      ...
      const tokens: Uint32Array = encoding.encode(text)
      return Array.from(tokens).map((t: number) => t.toString())
    }
